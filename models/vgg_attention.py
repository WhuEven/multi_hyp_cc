...
class VggAttention(VGG):
    ...
    def __init__(self, conf, pretrained=False, dropout = 0.5,
        ...
        if arch == 'vgg11':
            super(VggAttention, self).__init__(make_layers(cfgs['A']), **kwargs)#调用VGG中的构造函数，使用继承重写（定义不执行）
        elif arch == 'vgg11_bn':
            super(VggAttention, self).__init__(make_layers(cfgs['A'], batch_norm=True), **kwargs)
        else:
