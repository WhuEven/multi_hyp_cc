...
class VggClassificationAttention(VGG):
    ...
    def __init__(self, conf, pretrained=False, dropout = 0.5,
        ...
        if arch == 'vgg11':
            super(VggClassificationAttention, self).__init__(make_layers(cfgs['A']), **kwargs)#调用VGG中的构造函数，使用继承重写（定义不执行）
        elif arch == 'vgg11_bn':
            super(VggClassificationAttention, self).__init__(make_layers(cfgs['A'], batch_norm=True), **kwargs)
        else:
    ...
    def _inference(self, input, gt_illuminant, sensor, candidates, do_dropout = True):
        ...
        for i in range(candidates.shape[1]):#120
            ...
            # first conv (from VGG)
            x = self.conv1(input2) * self.attention(input2)
            x = self.relu1(x)
